...
- Safe Mode also disables auto clipboard sync and auto link opening.
...
- Default permissions keep `browse`/`download` on and leave write/sync/clipboard/power off until explicitly enabled.
...
- No cloud dependency.
- TLS is strict by default; plaintext fallback is an explicit compatibility mode.
- High-risk commands are rejected over plaintext connections.
