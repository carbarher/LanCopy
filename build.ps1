...
$solution = $null
if (Test-Path "SlskDownAvalonia\SlskDownAvalonia.slnx") {
    $solution = "SlskDownAvalonia\SlskDownAvalonia.slnx"
} elseif (Test-Path "SlskDown.sln") {
    $solution = "SlskDown.sln"
} else {
    Write-Error "No se encontró solución (SlskDownAvalonia.slnx ni SlskDown.sln)."
    exit 1
}
